class Library:
    ...
    def add_member(self, member):
        """Appends the provided member to the list of members"""
        self.members.append(member)

    def remove_member(self, member_id: int):
        """Removes the member with the provided ID from the list of members"""
        # Attempts parsing the ID to an integer.
        # If the parsing fails, the user will be informed, and the method will return False to indicate failure to remove the member.
    ...
    def issue_book(self, book_id, member):
        ...
        # If the parsing fails, the user will be informed, and the method will return False to indicate failure to issue the book.
        if self.util.parse_integer(book_id) is None:
            print(f"[Error] {book_id} is not a valid book ID.")
            return False
        # Parses the ID to an integer.
        ...
        if member.is_borrowed(book):
            print(f"[Error] {member.name} is already borrowing {book.title}.")
            return
        # Gets the number of copies available.
        copies = self.books.get(book)
        # Checks if any copies are available.
        if copies == 0:
            print(f"[Error] [{book.book_id}] {book.title} is out of stock.")
            return
    ...
    def return_book(self, book_id, member):
        ...
        # Attempts parsing ID to an integer.
        # If the parsing fails, the user will be informed, and the method will return False to indicate failure to return the book.
        if self.util.parse_integer(book_id) is None:
            print(f"[Error] {book_id} is not a valid book ID.")
            return False
        # Parses the ID to an integer.
    ...
    def get_book_from_string(self, name):
        ...
        for book in self.books:
            if book.title.lower() == name.lower():
                # Returns the book if a match was found.
                return book
    ...
    def get_member_from_string(self, name):
        ...
        for member in self.members:
            if member.name.lower() == name.lower():
                # Returns the member if a match is found.
                return member
        # Returns None to indicate that no member was found with the name provided.
        return None
    ...
    def display_member(self, member_id):
        ...
        if member is None:
            print(f"[Error] No member found with ID: {member_id}")
            return
        # Prints out the member's details.
        member.display_info()
    ...
    def get_book_from_id(self, book_id):
        ...
        # Checks if the library contains any books.
        if not self.contain_books(): return None
        # Iterates through all books in the library and compares IDs to find a match.
        for book in self.books:
            if book.book_id == book_id:
                # Returns the book if a match is fine.
                return book
    ...
    def get_copies(self, book_id):
        ...
        for book in self.books:
            # Compares IDs and returns the copies of the book if a match is found.
            if book.book_id == book_id:
    ...
    def is_borrowed(self, book):
        ...
        for member in self.members:
            ...
            # Returns False to indicate that the book isn't being lent to any members.
            return False
    ...
    def contain_books(self):
        ...
        if self.books is None:
            # Returns False if no books are present.
            return False
    ...
    def contain_members(self):
        ...
        if self.members is None:
            # Returns False if no members are present.
            return False
